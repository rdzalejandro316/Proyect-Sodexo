...
namespace WindowPV
{
    public partial class AddReferencia : Window
    {
        ...
        double cantidad = 0;
        double _desc = 0;
        double val_uni = 0;//val_uni         
        double subtotal = 0;//subtotal        
        double valIva = 0;//val_iv
        int valorIva = 0;
        double total = 0;//tot_tot                
        ...
        //public void llenarCampos(string referencia)
        //{
        //    CalcularDesc(referencia);

        //    cantidad = Convert.ToDecimal(TX_cantidad.Value);
        //   // MessageBox.Show("cantidad :" + cantidad);
        //    _desc = 1 - Convert.ToDecimal(procentaje_desc) / 100;
        //    val_uni = valor_ref * _desc / (1 + (iva) / 100);//val_uni 
        //    //Tx_ValUni.Text = val_uni.ToString();
        //    Tx_ValUni.Text = (string.Format(("{0:C}"), val_uni));
        //    subtotal = val_uni * cantidad;//subtotal
        //    Tx_SubTot.Text = (string.Format(("{0:C}"), subtotal));            
        //    Tx_PorDesc.Text = procentaje_desc.ToString();
        //    valIva = (subtotal * iva) / 100;//val_iv
        //    valorIva = Convert.ToInt32(valIva);
        //    Tx_valIva.Text = (string.Format(("{0:C}"), valorIva));
        //    total = subtotal + valorIva;//tot_tot
        //    Tx_TotTal.Text = (string.Format(("{0:C}"), total));
        //}
        ...
        private void AddRef_Click(object sender, RoutedEventArgs e)
        {
            ...
            Convert.ToDecimal(procentaje_desc), Convert.ToDecimal(iva), Convert.ToInt32(subtotal),
        ...
        public void insertRef(int idregcab, string cod_ref, string cod_bod, decimal cantidad, int val_uni, decimal val_iva, decimal por_des, decimal por_iva, int subtotal, string cod_sub, string cod_tiva, string cod_trn, string num_trn, string fecha_aded, decimal tot_tot)
        {
            ...
            using (SqlConnection connection = new SqlConnection(cnEmp))
            {
                using (SqlCommand cmd = connection.CreateCommand())
                {
                    try
                    {
                        cmd.CommandText = "INSERT INTO InCue_doc (idregcab,cod_ref,cod_bod,cantidad,val_uni,val_iva,por_des,por_iva,subtotal,cod_sub,cod_tiva,cod_trn,num_trn,fecha_aded,tot_tot) values (@idregcab,@cod_ref,@cod_bod,@cantidad,@val_uni,@val_iva,@por_des,@por_iva,@subtotal,@cod_sub,@cod_tiva,@cod_trn,@num_trn,@fecha_aded,@tot_tot)";
